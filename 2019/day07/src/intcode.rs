...
impl IntcodeMachine {
    ...
    pub fn run_until_input(&mut self) -> (bool, Vec<i32>) {
        let mut outputs = vec![];
    ...
    pub fn restore_with_input(&mut self, input: i32) -> (bool, Vec<i32>) {
        let mut outputs = vec![];
        let mut given = false;
        let mut got_to_input = false;
    ...
    pub fn run_collecting_outputs(&mut self, inputs: &[i32]) -> Vec<i32> {
        let mut outputs = vec![];
    ...
    fn run(&mut self, mut input_func: impl FnMut() -> (i32, bool), mut output_func: impl FnMut(i32)) {
        'program_loop: loop {
